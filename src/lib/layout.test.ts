...
import { MAX_CARD_HEIGHT_IN, MAX_FRAMES, MIN_FRAMES, OUTER_MARGIN_IN } from '../constants';
...
describe('computeLayout', () => {
  ...
  it('places the gutter on the bound (left) edge and the image to its right', () => {
    const l = computeLayout(baseOpts({ bindingSide: 'left', gutterIn: 0.5 }), 16 / 9);
    const s = l.slots[0];
    // Image starts a full gutter in from the card's left edge.
    expect(s.imgX - s.cardX).toBeCloseTo(0.5, 5);
  ...
  it('mirrors the gutter to the right edge for right binding', () => {
    ...
    expect(rightGap).toBeGreaterThan(0.3); // gutter + safe margin on the right
    expect(s.imgX - s.cardX).toBeCloseTo(0.1, 5); // only safe margin on the left
  });

  it('caps card height for very tall portrait clips (letterboxes instead of stretching)', () => {
    const l = computeLayout(baseOpts(), 9 / 16);
    expect(l.imgH).toBeLessThanOrEqual(MAX_CARD_HEIGHT_IN + 1e-9);
  ...
  it('keeps every card inside the printable area', () => {
    ...
    expect(last.cardX).toBeGreaterThanOrEqual(OUTER_MARGIN_IN - 1e-9);
    expect(last.cardX + l.cardW).toBeLessThanOrEqual(8.5 - OUTER_MARGIN_IN + 1e-9);
    expect(last.cardY + l.cardH).toBeLessThanOrEqual(11 - OUTER_MARGIN_IN + 1e-9);
  });
